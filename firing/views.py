...
from .models import *
from .forms import AccountForm, CreateUserForm, DetailForm, FireForm, FirerForm
...
from .filter import FireFilter, FirerFilter, ResultFilter
...
def dashboard(request):
    ...
    myfilter = FirerFilter(request.GET, queryset=firer)
    firer = myfilter.qs
    context = {'firer': firer, 'total_firer': total_firer,
               'passed': passed, 'failed': failed, 'appeared': appeared, 'myfilter': myfilter}
...
def result(request):
    ...
    firer = Firer.objects.all()
    myfilter = FireFilter(request.GET, queryset=fire)
    fire = myfilter.qs
    context = {'firer': firer, 'result': result,
               'fire': fire, 'myfilter': myfilter}
    return render(request, 'firing/result.html', context)


@login_required(login_url='home')
def fireview(request, pk):

    fire = Fire.objects.get(id=pk)
    firer = Firer.objects.all()
    context = {'firer': firer, 'fire': fire}
    return render(request, 'firing/fireview.html', context)
...
def detail(request):
    ...
    context = {'form': form}
    return render(request, 'firing/detail.html', context)


@login_required(login_url='home')
@allowed_users(allowed_roles=['admin'])
def addfirer(request):
    form = FirerForm
    if request.method == 'POST':
    ...
    context = {'form': form}
    return render(request, 'firing/addfirer.html', context)
...
def profilepage(request, pk):
    ...
    # fire = profile.fire_set.all()
    fire_num = fire_total.count()
    passed = fire_total.filter(remark='pass').count()

    context = {'fire_num': fire_num, 'passed': passed,
               'profile': profile, 'fire_total': fire_total}
...
def profile(request):
    results = request.user.firer.result_set.all()
    print('RESULTS', results)
    passed = results.filter(remark='pass').count()
    print(passed)
    result_num = results.count()
    context = {'results': results, 'result_num': result_num, 'passed': passed}
    return render(request, 'firing/profile-page.html', context)


@login_required(login_url='home')
# @allowed_users(allowed_roles=['firer'])
def accountsettings(request):
    user = request.user.firer
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
    context = {'form': form}
    return render(request, 'firing/accountsettings.html', context)


@login_required(login_url='home')
@allowed_users(allowed_roles=['firer'])
def profilefire(request, pk):
    f = Result.objects.get(id=pk)

    context = {'f': f}
    return render(request, 'firing/profile-fire.html', context)


@login_required(login_url='home')
def proflieanalytics(request):
    results = request.user.firer.result_set.all()

    # firer = Firer.objects.all()
    total_firer = results.count()
    passed = results.filter(remark='pass').count()
    failed = results.filter(remark='fail').count()
    # not_appeared = firer.filter(status='Not appeared').count()
    # appeared = total_firer - not_appeared
    context = {'results': results, 'total_firer': total_firer,
               'passed': passed, 'failed': failed, }
    return render(request, 'firing/profileanalytics.html', context)
...
def analytics(request):
    ...
    appeared = total_firer - not_appeared
    context = {'firer': firer, 'total_firer': total_firer,
               'passed': passed, 'failed': failed, 'appeared': appeared, 'not_appeared': not_appeared}
    return render(request, 'firing/analytics.html', context)
...
def weapon(request):
    ...
    weapon_num = weapon.count()
    z = weapon.filter(zeroed=True).count()
    cls1 = weapon.filter(cls='One').count()
    blr = weapon.filter(cls='Five').count()

    context = {'weapon_num': weapon_num, 'cls1': cls1,
               'blr': blr, 'z': z, 'weapon': weapon}
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for box in bboxes:
                ...
                final_box.append([ymin, xmax, ymax, ymin])

            distance = []
            for i in range(5):
                for j in range(5):
                    temp = (((boxes[i][1]-boxes[j][1])*(boxes[i][1]-boxes[j][1])) +
                            ((boxes[i][0]-boxes[j][0])*(boxes[i][0]-boxes[j][0])))
                    distance.append([math.sqrt(temp)])

            group = grouping(distance)
            fire.grouping = group

            fire.save()

            if total > 2:
                r = Result(firer=fire.detail.target_1,
                           fire=fire, remark="pass")
                r.save()
            else:
                r = Result(firer=fire.detail.target_1,
                           fire=fire, remark="fail")
                r.save()
